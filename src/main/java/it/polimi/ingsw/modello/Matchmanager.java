...
public abstract class Matchmanager{
  ...
  public void startGame(){
    ...
    for(Player player: match.getPlayers()){
      player.getView().sendCommonCard(match.getCommonCards());
      player.getView().updateboard(match.getBoard());
    }
    int position = (int)(Math.random() * 4);
    match.setChair(match.getPlayers().get(position));
    match.getChair().getView().assignedChair(match.getChair().getNickname());
  }
  /**
  ...
  public void setPersonalGoal(ArrayList<Player> p){
    ...
    for(Player player:p){
      ...
      PersonalGoalCard card=new PersonalGoalCard(rand.get(r).getPersonalObjective());
      //take an objective from a PersonalGoal in rand
      player.setPersonalCard(card);
      player.getView().sendPersonalCard(card);
